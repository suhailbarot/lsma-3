...
f1 = open('cnn_pred/kaggle_test_P001.csv')
f2 = open('cnn_pred/kaggle_test_P002.csv')
f3 = open('cnn_pred/kaggle_test_P003.csv')
...
for i,j,k in zip(nr1,nr2,nr3):
	print np.max(np.array([i[1],j[1],k[1]]))
	if np.max(np.array([sr1*i[1],sr2*j[1],sr3*k[1]]))*7.5<0.5:
		tag = 0
	else:
