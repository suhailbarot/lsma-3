...
if __name__ == '__main__':
    ...
    svm_classifier = SVC(gamma='scale',kernel="rbf",probability=True)
    ...
    for line in train_files:
        ...
        f_file_2 = open(sys.argv[5]+file+'.txt')
        X_train.append(np.array(map(float,f_file_1.read().strip().split()+map(float,f_file_2.read().strip().split()))))
        if tag==event_name:            
            Y_train.append(1)
        else:
            Y_train.append(0)
