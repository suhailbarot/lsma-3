...
if __name__ == '__main__':
    ...
    for line in val_files_list.readlines():
        ...
        f_file_1 = open(sys.argv[2]+file+'.txt')
        f_file_2 = open(sys.argv[6]+file+'.txt')
        X_val.append(np.array(map(float,f_file_1.read().strip().split()+map(float,f_file_2.read().strip().split()))))
        # f_file.close()
        # if tag=="NULL":
        #     Y_train.append(0)
        # else:
    ...
    output_file = open('cnn_asr_pred/kaggle_test_'+sys.argv[5]+'.csv','w+')
    ...
    for line in test_files_list.readlines():
        ...
        f_file_1 = open(sys.argv[2]+file+'.txt')
        f_file_2 = open(sys.argv[6]+file+'.txt')
        X_test.append(np.array(map(float,f_file_1.read().strip().split()+map(float,f_file_2.read().strip().split()))))
        # f_file.close()
        # if tag=="NULL":
        #     Y_train.append(0)
        # else:
